...
public class ThermosProxyServlet extends AsyncMiddleManServlet {
  ...
  static final Pattern agentPattern = Pattern.compile(
      "^/thermos/agent/(?<agent>[a-zA-Z0-9-]+)/?(?<path>.*)");
  private static final Cache<String, String> cacheAgentId =
  ...
  protected void service(final HttpServletRequest request, final HttpServletResponse response)
    ...
    final Matcher hostMatcher = agentPattern.matcher(path);
  ...
  protected String rewriteTarget(HttpServletRequest clientRequest) {
    if (!validateDestination(clientRequest.getServerName(), clientRequest.getServerPort())) {
      log.debug("Invalid clientRequest: {}:{}", clientRequest.getServerName(), clientRequest.getServerPort());
      return null;
    }
    // build url
    final URL url;
  ...
  private Iterable<IHostAttributes> getHostAttributes() {
    return getStorage().read(storeProvider -> storeProvider.getAttributeStore().getHostAttributes());
  }
  ...
  static class ThermosContentTransformer extends AfterContentTransformer {
    ...
    public boolean transform(Source source, Sink sink) throws IOException {
      ...
      int read = 0;
      try (final InputStream s = source.getInputStream();
           final OutputStream o = sink.getOutputStream()) {
